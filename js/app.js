...
function checkAndUnlockLevels() {
  // 所有 level 都允许跳级访问，全部标记为 unlocked
  for (let i = 0; i < COURSES.levels.length; i++) {
...
function getLessonStatus(lessonId) {
  ...
  // 每个 level 的第一个未完成课程是 current（可点击），其余 locked
  // 这样实现了：可以跳级到任意 level 的第一个节点，但 level 内部必须顺序推进
  for (const lesson of level.lessons) {
    if (!appState.completedLessons.includes(lesson.id)) {
      return lesson.id === lessonId ? 'current' : 'locked';
    }
  }
  return 'locked';
}
...
function showLockedLessonTooltip(lessonId) {
  ...
  overlay.innerHTML = `
    <div class="locked-tooltip">
      ...
      <div class="tooltip-hint">请先完成本级别前面的课程才可以解锁哦！</div>
    </div>`;
  document.body.appendChild(overlay);
